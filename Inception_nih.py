...
checkpoint_callback = ModelCheckpoint(filepath='inception_model_checkpoint_nih_nw.h5',
                                      save_best_only=True,
...
for i, partition_generator in enumerate([partition_1_generator, partition_2_generator, partition_3_generator]):
    weights_filepath = 'inception_model_checkpoint_nih_nw.h5'
...
model.save('inception_NIH_Model_nw.h5')
...
# Save all history
with open('inception_NIH_all_history_nw.pkl', 'wb') as file:
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('inception_loss_curve_nih_nw.png')

model = load_model('inception_NIH_Model_nw.h5')
...
plt.tight_layout()
plt.savefig('inception_confusion_matrix_nih_nw.png')
