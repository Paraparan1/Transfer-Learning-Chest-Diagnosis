...
checkpoint_callback = ModelCheckpoint(filepath='xception_model_checkpoint_nih.h5',
                                      save_best_only=True,
...
for i, partition_generator in enumerate([partition_1_generator, partition_2_generator, partition_3_generator]):
    weights_filepath = 'xception_model_checkpoint_nih.h5'
...
model.save('xception_NIH_Model.h5')
...
# Save all history
with open('xception_NIH_all_history.pkl', 'wb') as file:
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('xception_loss_curve_nih.png')

model = load_model('xception_NIH_Model.h5')
...
plt.tight_layout()
plt.savefig('xception_confusion_matrix_nih.png')
