...
checkpoint_callback = ModelCheckpoint(filepath='resNet_model_checkpoint_nw.h5',
                                      save_best_only=True,
...
for i, partition_generator in enumerate([partition_1_generator, partition_2_generator, partition_3_generator]):
    if i > 0:
        ...
        model.load_weights('resNet_model_checkpoint_nw.h5')
    ...
    if i != 2:
        ...
        # Save the model
        model.save("resNet_NIH_Model_nw.h5")


# Save all history to a file using pickle
with open('resNet_all_history_nw.pkl', 'wb') as file:
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('resNet_loss_curve_NIH_nw.png')

model = load_model('resNet_NIH_Model_nw.h5')
...
plt.savefig('resNet_confusion_matrix_NIH_nw.png')
